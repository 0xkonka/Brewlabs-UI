...
const getFarmFromTokenSymbol = (farms, tokenSymbol, preferredQuoteTokens = ["BUSD", "WBNB"]) => {
...
const getFarmQuoteTokenPrice = (farm, quoteTokenFarm, bnbPriceBusd) => {
  ...
  if (quoteTokenFarm.quoteToken.symbol === "WBNB") {
    return quoteTokenFarm.tokenPriceVsQuote
      ? bnbPriceBusd.mulUnsafe(FixedNumber.from(quoteTokenFarm.tokenPriceVsQuote))
      : FIXED_ZERO;
  }
...
const getFarmsPrices = (farms) => {
  ...
  const bnbPriceBusd = bnbBusdFarm.tokenPriceVsQuote
    ? FIXED_ONE.divUnsafe(FixedNumber.from(bnbBusdFarm.tokenPriceVsQuote))
    : FIXED_ONE;
