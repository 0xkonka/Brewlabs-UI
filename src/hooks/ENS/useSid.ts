...
import { useSIDContract } from "hooks/useContract";
...
export const useSidNameForAddress = (address: string, fetchData = true) => {
  ...
  const sidContract = useSIDContract(chainId, getSidAddress(chainId));

  const { data: sidName, isLoading } = useSWRImmutable(
    ...
    async () => {
      ...
      if (parseInt(resolverAddress.toString(), 16) === 0) {
        return {
          name: null,
        };
      }
      const resolverContract = getSidResolverContract(chainId, resolverAddress.toString());
  ...
  return useMemo(() => {
    return { sidName: sidName?.name, isLoading };
  }, [sidName, isLoading]);
};
